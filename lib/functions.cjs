...
const { Data, Delta, Step, CustomError } = require('./types.cjs');
...
function getInstructionParameters(requirements, stack) {
    let flag = false;
    if (requirements[0]) {
        const reqSize = requirements[1].reduce((prev, cur) => prev.length > cur.length ? prev : cur).length;
...
global.applyCOMPARE = (instruction, parameters, stack) => {
    ...
    switch (parameters[0].prim) {
        ...
        case "signature":
        case "chain_id":
            r.value.push(parameters[0].value[0].localeCompare(parameters[1].value[0]).toString());
            break;
        default:
            throw new CustomError("COMPARE not implemented for complex types", {instruction, parameters});
    }
    return r;
};
...
global.applyDIP = (instruction, parameters, stack) => {
    var n = 1;
    if (instruction.args[0].hasOwnProperty("int")) {
        n = parseInt(instruction.args[0].int);
